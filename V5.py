...
# Giám sát thư mục (Tính năng mới)
...
VSF_EXECUTABLE_NAME = "VideoSubFinderWXW_intel.exe"
LOG_DATETIME_FORMAT = "%H:%M:%S"
LOG_FILE_DATETIME_FORMAT = "%Y-%m-%d %H:%M:%S"

# --- Lớp xử lý sự kiện file cho Watchdog ---
class VSFProgressHandler(FileSystemEventHandler):
    """Giám sát thư mục RGBImages và cập nhật tiến trình."""
    ...
    def on_created(self, event):
        ...
        self.app.root.after(0, self.app.update_vsf_progress_from_handler, file_name, self.file_count)
...
class VideoOCRApp:
    def __init__(self, root):
        ...
        self.video_duration_str = "00:00:00"
        self.vsf_process = None
        self.observer = None
        ...
        self.current_vsf_output_path = None

        # --- Khởi tạo UI và tải cấu hình ---
        self.init_ui()
        self.load_config()
        self.log_message(f"Chào mừng đến với {APPLICATION_NAME} V5.0!")
        self.log_message("Vui lòng đảm bảo file 'credentials.json' từ Google Cloud đã ở trong cùng thư mục.")
        self.log_message("Nếu là lần đầu chạy, một cửa sổ trình duyệt sẽ mở ra để bạn đăng nhập và cấp quyền.")

    def init_ui(self):
        """Khởi tạo toàn bộ giao diện người dùng."""
        self.root.title(f"{APPLICATION_NAME} v5.0 (Tích hợp Watchdog)")
        ...
        vsf_path_frame = tk.Frame(config_frame)
        vsf_path_frame.pack(fill="x", pady=2)
        tk.Label(vsf_path_frame, text="Đường dẫn VSF:", width=15, anchor="w").pack(side="left")
        self.videosubfinder_path_var = tk.StringVar()
        tk.Entry(vsf_path_frame, textvariable=self.videosubfinder_path_var).pack(side="left", expand=True, fill="x", padx=(0, 5))
        tk.Button(vsf_path_frame, text="...", command=self.browse_for_vsf).pack(side="left")
        ...
        self.coord_label = tk.Label(coord_display_frame, text="Tọa độ chưa được chọn | Thời lượng: --:--:--", anchor="w", fg="grey")
        self.coord_label.pack(side="left")

        vsf_run_frame = tk.Frame(workflow_frame)
        vsf_run_frame.pack(fill="x", pady=3)
        self.vsf_button = tk.Button(vsf_run_frame, text="3. Chạy VSF", command=self.run_vsf_processing, state=tk.DISABLED, width=15)
        self.vsf_button.pack(side="left", padx=5)
        ...
        self.create_txtimages_var = tk.BooleanVar()
        ...
        tk.Checkbutton(options_frame, text="Tạo TXTImages (thay vì RGB)", variable=self.create_txtimages_var).pack(side="left", padx=10)
        tk.Checkbutton(options_frame, text="Xóa hết file tạm khi xong", variable=self.delete_all_temp_var).pack(side="left", padx=10)

        # --- Frame Trạng thái và Log ---
    ...
    def select_video_and_setup(self):
        video_file = filedialog.askopenfilename(
            title="Chọn một tệp video",
            filetypes=[("Video files", "*.mp4 *.avi *.mkv *.mov")]
        )
        if not video_file:
            self.log_message("⚠️ Không có video nào được chọn.", "warning")
            return
        ...
        # Lấy và hiển thị thời lượng video
        self.get_video_duration(video_file)
        self.coord_label.config(text=f"Tọa độ chưa được chọn | Thời lượng: {self.video_duration_str}")
    ...
    def run_vsf_processing(self):
        vsf_path = self.videosubfinder_path_var.get()
        if not os.path.exists(vsf_path):
            messagebox.showerror("Lỗi", f"Không tìm thấy VideoSubFinder tại: {vsf_path}")
            return
        self.set_ui_state_for_processing(True)
        self.update_status("Đang chuẩn bị chạy VideoSubFinder...")
        threading.Thread(target=self._vsf_thread_target, daemon=True).start()

    def _vsf_thread_target(self):
        video_path = Path(self.current_video_path)
        self.current_vsf_output_path = video_path.parent / f"{video_path.stem}_out"
        
        output_folder = "TXTImages" if self.create_txtimages_var.get() else "RGBImages"
        # Monitor RGBImages folder for most accurate progress tracking
        monitor_folder = self.current_vsf_output_path / "RGBImages"
        self.current_images_path = self.current_vsf_output_path / output_folder
        
        # Remove old output folder if exists to ensure accurate monitoring
        if self.current_vsf_output_path.exists():
            try:
                shutil.rmtree(self.current_vsf_output_path)
                self.log_message(f"🧹 Đã xóa thư mục output cũ: {self.current_vsf_output_path}")
            except Exception as e:
                self.log_message(f"⚠️ Không thể xóa thư mục output cũ: {e}", "warning")
        
        monitor_folder.mkdir(parents=True, exist_ok=True)

        vsf_executable_path = self.videosubfinder_path_var.get()
        
        # Get the directory containing the VSF executable for cwd
        vsf_directory = os.path.dirname(vsf_executable_path)

        command = [
            vsf_executable_path, "-c", "-r",
            "-i", str(video_path),
            "-o", str(self.current_vsf_output_path),
            "-te", self.crop_top_var.get(), "-be", self.crop_bottom_var.get(),
            "-le", self.crop_left_var.get(), "-re", self.crop_right_var.get(),
        ]
        if self.create_txtimages_var.get():
            command.insert(2, "-ccti")

        try:
            self.log_message("🚀 Bắt đầu chạy VideoSubFinder...")
            self.log_message(f"Lệnh: {' '.join(command)}")
            self.log_message(f"Working directory: {vsf_directory}")

            # Start folder monitoring
            event_handler = VSFProgressHandler(self)
            self.observer = Observer()
            self.observer.schedule(event_handler, str(monitor_folder), recursive=False)
            self.observer.start()

            # **CRITICAL FIX: Use proper cwd parameter**
            self.vsf_process = subprocess.Popen(
                command, 
                ...
                encoding='utf-8', 
                errors='ignore', 
                cwd=vsf_directory,  # Set working directory to VSF folder
                bufsize=1,
                creationflags=subprocess.CREATE_NO_WINDOW
            )
            
            # Read logs from VSF in separate threads
            stdout_thread = threading.Thread(target=self.log_stream, args=(self.vsf_process.stdout,))
            stderr_thread = threading.Thread(target=self.log_stream, args=(self.vsf_process.stderr, "error"))
            stdout_thread.start()
            stderr_thread.start()

            self.vsf_process.wait()
            stdout_thread.join()
            stderr_thread.join()

            return_code = self.vsf_process.returncode

            if self.stop_flag.is_set():
                self.log_message("✋ Tiến trình VSF đã được dừng.", "warning")
                return

            if return_code == 0:
                self.log_message("✅ VSF đã hoàn tất xử lý.")
                # Ensure progress bar and status update on success
                self.root.after(0, self.progress_bar.config, {"value": 100})
                self.update_status("VSF hoàn tất! Sẵn sàng để OCR.")
                self.root.after(0, self.start_button.config, {"state": tk.NORMAL})
            else:
                self.log_message(f"❌ VSF kết thúc với mã lỗi: {return_code}", "error")

        except FileNotFoundError:
            self.log_message(f"❌ Lỗi: Không tìm thấy file thực thi VSF tại '{vsf_executable_path}'", "error")
        except Exception as e:
            self.log_message(f"❌ Lỗi hệ thống khi chạy VSF: {e}", "error")
        finally:
            if self.observer:
                self.observer.stop()
                self.observer.join()
    ...
    def log_stream(self, stream, log_type="info"):
        """Helper method to read and log subprocess output streams"""
        try:
            for line in iter(stream.readline, ''):
                ...
                if line:  # Only log non-empty lines
                    self.log_message(line, log_type)
        except Exception as e:
            self.log_message(f"Lỗi khi đọc stream: {e}", "error")
    ...
    def stop_processing(self):
        ...
        # Dừng tiến trình VSF
        if self.vsf_process and self.vsf_process.poll() is None:
            try:
                # Sử dụng psutil để dừng cả tiến trình con
                parent = psutil.Process(self.vsf_process.pid)
                for child in parent.children(recursive=True):
                    child.terminate()
                parent.terminate()
                self.log_message("✋ Đã dừng tiến trình VideoSubFinder.", "warning")
            except psutil.NoSuchProcess:
                pass
        
        # Dừng luồng giám sát watchdog
        if self.observer:
            self.observer.stop()
            self.observer.join()
            self.observer = None
            self.log_message("✋ Đã dừng giám sát thư mục.", "warning")
    ...
    def get_video_duration(self, video_path):
        """Lấy thời lượng video bằng OpenCV và lưu lại."""
        try:
            cap = cv2.VideoCapture(video_path)
            if not cap.isOpened():
                self.log_message("Lỗi: Không thể mở file video.", "error")
                return
            fps = cap.get(cv2.CAP_PROP_FPS)
            frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            self.video_duration_seconds = frame_count / fps if fps > 0 else 0
            cap.release()
            
            td = datetime.timedelta(seconds=self.video_duration_seconds)
            self.video_duration_str = str(td).split('.')[0]
            self.log_message(f"Thời lượng video: {self.video_duration_str}")
        except Exception as e:
            self.log_message(f"Lỗi khi lấy thời lượng video: {e}", "error")
            self.video_duration_seconds = 0
            self.video_duration_str = "Không xác định"

    def update_vsf_progress_from_handler(self, filename, file_count):
        """Callback được gọi từ Watchdog để cập nhật UI."""
        match = re.search(r"(\d+)_(\d+)_(\d+)", filename)
        if not match or self.video_duration_seconds == 0:
            return

        h, m, s = map(int, match.groups())
        current_seconds = h * 3600 + m * 60 + s
        
        percentage = (current_seconds / self.video_duration_seconds) * 100
        percentage = max(0, min(percentage, 100))

        remaining_seconds = self.video_duration_seconds - current_seconds
        time_remaining_str = str(datetime.timedelta(seconds=remaining_seconds)).split('.')[0]
        
        self.progress_bar['value'] = percentage
        self.status_label.config(text=f"⚙️ VSF: {percentage:.1f}% | Còn lại: {time_remaining_str} | Ảnh: {file_count}")

    # --- Các hàm khác không thay đổi (load_config, save_config, on_exit, get_credentials, v.v...) ---
    # ... (Dán các hàm còn lại của ORCV4.1.py vào đây)
    # --- 1. Khởi tạo và Cấu hình ---
    
    def load_config(self):
        """Tải cài đặt từ file config.ini khi khởi động."""
        config = configparser.ConfigParser()
        
        if not os.path.exists(CONFIG_FILE):
            config['Settings'] = {
                "folder_id": "", "delete_all_temp": "True", "nen_raw_texts": "False",
                "videosubfinder_path": "", "threads": "10"
            }
            config['Presets'] = {
                'Default Preset 1': '1.0000,0.1500,0.0000,1.0000',
                'Default Preset 2': '0.7500,0.2000,0.1000,0.9000'
            }
        ...
        if not config.has_section('Presets'): config.add_section('Presets')

        settings = config['Settings']
        self.folder_id = settings.get("folder_id", "")
        self.delete_all_temp_var.set(settings.getboolean("delete_all_temp", fallback=True))
        self.nen_raw_texts_var.set(settings.getboolean("nen_raw_texts", fallback=False))
        self.videosubfinder_path_var.set(settings.get("videosubfinder_path", ""))
        self.threads_var.set(settings.get("threads", "10"))

        self.presets.clear()
        presets_section = config['Presets']
        for name, values_str in presets_section.items():
            try:
                values = [float(v.strip()) for v in values_str.split(',')]
                if len(values) == 4: self.presets[name] = values
            except (ValueError, TypeError):
                self.log_message(f"Lỗi đọc preset '{name}'.", "warning")
        self.update_preset_combobox()

    def save_config(self):
        """Lưu các cài đặt hiện tại vào file config.ini."""
        config = configparser.ConfigParser()
        config.read(CONFIG_FILE, encoding='utf-8')

        if not config.has_section('Settings'): config.add_section('Settings')
        
        config['Settings']["folder_id"] = self.folder_id
        config['Settings']["delete_all_temp"] = str(self.delete_all_temp_var.get())
        config['Settings']["nen_raw_texts"] = str(self.nen_raw_texts_var.get())
        config['Settings']["videosubfinder_path"] = self.videosubfinder_path_var.get()
        config['Settings']["threads"] = self.threads_var.get()
        
        if config.has_section('Presets'): config.remove_section('Presets')
        config.add_section('Presets')
        
        for name, values in self.presets.items():
            config['Presets'][name] = ','.join(f"{v:.4f}" for v in values)
    ...
    # --- 2. Xác thực và API Google Drive ---
    ...
    def find_or_create_drive_folder(self):
        try:
            ...
            if files:
                self.log_message(f"✅ Tìm thấy thư mục '{DRIVE_FOLDER_NAME}'.")
                return files[0].get('id')
            else:
                self.log_message(f"⚠️ Không tìm thấy '{DRIVE_FOLDER_NAME}', đang tạo mới...")
                file_metadata = {'name': DRIVE_FOLDER_NAME, 'mimeType': 'application/vnd.google-apps.folder'}
                folder = service.files().create(body=file_metadata, fields='id').execute()
                self.log_message(f"✅ Đã tạo thư mục mới (ID: {folder.get('id')}).")
                return folder.get('id')
        except HttpError as error:
            self.log_message(f"❌ Lỗi Google Drive: {error}", "error")
            return None
        except Exception as e:
            self.log_message(f"❌ Lỗi không xác định với Google Drive: {e}", "error")
            return None

    # --- 3. Quy trình chính (Workflow) ---
    ...
    def update_crop_values(self, top, bottom, left, right):
        ...
        self.log_message(f"🎯 Đã nhận tham số VSF: Top={top:.4f}, Bottom={bottom:.4f}, Left={left:.4f}, Right={right:.4f}")
        self.update_status("Sẵn sàng chạy VSF")
        self.coord_label.config(text=f"Tọa độ: T={top:.2f}, B={bottom:.2f}, L={left:.2f}, R={right:.2f} | Thời lượng: {self.video_duration_str}")

    def start_processing_workflow(self):
        if not self.current_images_path or not os.path.exists(self.current_images_path):
            messagebox.showwarning("Cảnh báo", "Chưa có thư mục ảnh. Vui lòng chạy VSF trước.")
            return
        try:
    ...
    def _ocr_thread_target(self):
        ...
        if not self.folder_id:
            self.folder_id = self.find_or_create_drive_folder()
            if not self.folder_id:
                self.root.after(0, self.set_ui_state_for_processing, False)
                return
            self.save_config()
    ...
    def ocr_image(self, image_path, line_number, current_directory):
        ...
        try:
            ...
            start_time = f"{start_h}:{start_m}:{start_s},{start_ms[:3]}"
            end_time = f"{end_h}:{end_m}:{end_s},{end_ms[:3]}"
    ...
    def finalize_processing(self, raw_texts_dir, texts_dir):
        ...
        if self.delete_all_temp_var.get():
            try:
                ...
                if self.current_vsf_output_path and Path(self.current_vsf_output_path).exists():
                    shutil.rmtree(self.current_vsf_output_path)
    ...
    def set_ui_state_for_processing(self, is_processing):
        ...
        self.vsf_button.config(state=tk.DISABLED if is_processing or not self.crop_top_var.get() else tk.NORMAL)
    ...
    def progress_callback(self):
        with self.progress_lock:
            ...
            if self.total_images > 0:
                ...
                self.root.after(0, self.update_status, f"⚙️ Đang OCR: {self.completed_scans}/{self.total_images}")
    ...
    def log_message(self, message, level="info"):
        def _log():
            ...
            final_level = "success" if message.startswith("✅") or message.startswith("🎉") else level
    ...
    def browse_for_vsf(self):
        path = filedialog.askopenfilename(title="Chọn file VideoSubFinderWXW_intel.exe", filetypes=[("Executable", "*.exe")])
        if path:
            self.videosubfinder_path_var.set(path)
            self.log_message(f"Đã cập nhật đường dẫn VSF: {path}")
    ...
    def manage_presets(self):
        manager_window = PresetManager(self.root, self.presets, self.update_presets_callback)
...
# --- Các lớp phụ (CropSelectorApp, PresetManager) không thay đổi đáng kể ---
# ... Dán mã nguồn của 2 lớp này từ file ORCV4.1.py vào đây ...
class CropSelectorApp:
    ...
    def show_frame(self, frame_index):
        ...
        self.update_time_display(frame_index / self.fps)

    def get_pixel_coords(self, event):
        x = (event.x - self.offset_x) * (self.video_width / self.display_w)
        y = (event.y - self.offset_y) * (self.video_height / self.display_h)
        return x, y
    ...
    def update_time_display(self, current_time_sec):
        ...
        self.time_label.config(text=f"{td.seconds // 3600:02}:{(td.seconds // 60) % 60:02}:{td.seconds % 60:02}.{td.microseconds // 1000:03}")

    def draw_bounding_lines(self):
        cx = lambda x: (x * (self.display_w / self.video_width)) + self.offset_x
        cy = lambda y: (y * (self.display_h / self.video_height)) + self.offset_y
...
class PresetManager:
    ...
    def add_preset(self):
        ...
        if not name or name in self.presets:
            if name: messagebox.showerror("Lỗi", f"Preset '{name}' đã tồn tại.", parent=self.top)
            return
